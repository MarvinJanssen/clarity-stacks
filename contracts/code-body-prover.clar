...
(define-constant chain-id-bytes 0x00000001) ;;(unwrap-panic (slice? (unwrap-panic (to-consensus-buff? chain-id)) u13 u17)))
(define-constant tx-version 0x00) ;;(if is-in-mainnet 0x00 0x80))
...
(define-read-only (serialized-tx (nonce (buff 8))
	(fee (buff 8))
	(signature (buff 65))
	(contract principal)
	...
	(let
		(
			(principal-data (match (principal-destruct? contract) success success error error))
			(contract-name (unwrap-panic (get name principal-data)))
		)
		(concat tx-version
...
(define-read-only (calculate-txid
	...
	(ok (sha512/256
		(serialized-tx nonce fee signature contract code-body))))
)
