...
import {
    block_header_hash,
    block_index_header_hash,
    // fetch_raw_nakamoto_block,
    merkle_tree_from_txs,
    parse_raw_nakamoto_block,
    proof_path_to_cv
} from "./clarity-stacks.ts";

export async function example() {
    // const raw_block = await fetch_raw_nakamoto_block(438372);
    ...
    const block_sighash = block_header_hash(block);

    const block_id = block_index_header_hash(block_sighash, block.consensus_hash);

    const merkle_tree = merkle_tree_from_txs(block.transactions);

    console.log('block id                ', bytesToHex(block_id));
    console.log('block hash              ', bytesToHex(block_sighash));
    console.log('calculated merkle root  ', bytesToHex(merkle_tree.root()));
    console.log('expected merkle root    ', bytesToHex(block.tx_merkle_root));

    console.log('tx1 proof', merkle_tree.proof(0).map(bytesToHex));
    console.log('tx2 proof', merkle_tree.proof(1).map(bytesToHex));

    console.log('proof CV', proof_path_to_cv(1, merkle_tree.proof(1), 1));
